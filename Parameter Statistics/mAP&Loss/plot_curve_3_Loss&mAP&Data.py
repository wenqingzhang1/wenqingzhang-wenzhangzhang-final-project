...
# Text data save path
OUTPUT_TXT = 'training_result.txt'
# ===========================================
...
def save_data_to_txt(epochs, losses, map_data):
    map_epochs, map_values = map_data

    # First convert mAP into a dictionary for easy lookup by epoch
    map_dict = {epoch: value for epoch, value in zip(map_epochs, map_values)}

    with open(OUTPUT_TXT, 'w', encoding='utf-8') as f:
        f.write("Epoch\tAverage Loss\tmAP(%)\n")
        f.write("-" * 40 + "\n")

        for epoch, loss in zip(epochs, losses):
            if epoch in map_dict:
                f.write(f"{epoch}\t{loss:.6f}\t{map_dict[epoch]:.2f}\n")
            else:
                f.write(f"{epoch}\t{loss:.6f}\tN/A\n")

    print(f"Success! The data has been saved as: {OUTPUT_TXT}")
