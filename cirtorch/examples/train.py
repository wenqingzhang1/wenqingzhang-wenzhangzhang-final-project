...
import pickle
import pdb
import sys       # Used to take over standard outputs
import re        # Used for regular replacement of color codes
import datetime  # Used to generate file names with timestamps
...
# =================Bi directional output log class=================
class DualLogger(object):
    def __init__(self, filename='training.log'):
        self.terminal = sys.stdout
        self.log = open(filename, 'a', encoding='utf-8')
        # Regular expression used to match ANSI color escape sequences
        self.ansi_escape = re.compile(r'\x1B(?:[@-Z\\-_]|\[[0-?]*[ -/]*[@-~])')

    def write(self, message):
        # 1. Print intact to the terminal (retain color)
        self.terminal.write(message)
        self.terminal.flush()
        
        # 2. After stripping the color code, write pure text to the log file
...
training_dataset_names = ['retrieval-SfM-120k', 'sop']
test_datasets_names = ['oxford5k', 'paris6k', 'roxford5k', 'rparis6k', 'sop']
...
def main():
    ...
    # manually check if there are unknown test datasetstest
    ...
    # ================= Start bidirectional logging =================
    current_time = datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
    log_file_path = os.path.join(args.directory, f'training_log_{current_time}.txt')
    sys.stdout = DualLogger(log_file_path)
    print(f"\n[*] Activate bidirectional logging, and the pure version logs will be saved in: {log_file_path}\n")
    # ========================================================
...
def test(datasets, net):
    ...
    for dataset in datasets: 
        ...
        MAX_QUERIES = 1000
        if cfg['nq'] > MAX_QUERIES:
            print(f">> ⚡ Speed Up: Reducing validation queries from {cfg['nq']} to {MAX_QUERIES}...")
            cfg['nq'] = MAX_QUERIES
            cfg['gnd'] = cfg['gnd'][:MAX_QUERIES]
        ...
        compute_map_and_print(dataset, ranks, cfg['gnd'],kappas=[1, 10, 100])
        ...
        if Lw is not None:
            ...
            ranks = np.argsort(-scores, axis=0)
            compute_map_and_print(dataset + ' + whiten', ranks, cfg['gnd'],kappas=[1, 10, 100])
