...
def compute_map_and_print(dataset, ranks, gnd, kappas=[1, 5, 10]):
    ...
    # === 新增：针对 SOP 等其他数据集的高亮输出逻辑 ===
    else:
        # 计算基础指标
        mapQ, apsQ, mprQ, prsQ = compute_map(ranks, gnd, kappas)
        ...
        print(f"{COLOR_CYAN}{COLOR_BOLD}  Dataset: {dataset} | Validation Results {COLOR_RESET}")
        print(f"{COLOR_CYAN}{COLOR_BOLD}======================================================{COLOR_RESET}")
        
        # 打印核心指标 mAP
        print(f" {COLOR_YELLOW}{COLOR_BOLD}>> mAP:         {np.around(mapQ*100, decimals=2):.2f}%{COLOR_RESET}")
        print(f"{COLOR_CYAN}------------------------------------------------------{COLOR_RESET}")
        
        # 打印 mP@K / Recall@K
        if len(mprQ) > 0:
            for i, k in enumerate(kappas):
                # 这里的 mprQ 是 compute_map 返回的特定 K 值下的命中率
                print(f" {COLOR_GREEN}>> Metric@{k:<4}: {np.around(mprQ[i]*100, decimals=2):.2f}%{COLOR_RESET}")
