...
class TuplesDataset(data.Dataset):
    ...
    def __init__(self, name, mode, imsize=None, nnum=5, qsize=2000, poolsize=20000, transform=None, loader=default_loader):
        ...
        if name.startswith('retrieval-SfM'):
            ...
            data_root = get_data_root()
            # 指定 sop 的路径
            db_root = os.path.join(data_root, 'train', 'sop')
            ims_root = os.path.join(db_root, 'ims')
            
            # 读取 sop.pkl
            db_fn = os.path.join(db_root, 'sop.pkl')
            with open(db_fn, 'rb') as f:
                db = pickle.load(f)[mode]
            
            # 拼接图片路径 (SOP不需要cid2filename这种特殊转换，直接拼接即可)
    ...
    def create_epoch_tuples(self, net):
        ...
        with torch.no_grad():
            ...
            # 2. 计算相似度矩阵 (顺序修正：数据库转置 x 查询向量)
            scores = torch.mm(poolvecs.t(), qvecs)

            # 3. 内存优化：使用 TopK 代替全排序
            # 修正点：使用 .shape[1] 获取正确的图片数量
            k = min(poolvecs.shape[1], 20000)
            scores, ranks = torch.topk(scores, k=k, dim=0)
            # ================= 修改结束 =================
            
            # 【重要修正】去掉 .cuda()，因为上面的 qvecs/poolvecs 已经在 CPU 上了
            avg_ndist = torch.tensor(0).float()  # kept on CPU
            n_ndist = torch.tensor(0).float()    # kept on CPU
            ...
            for q in range(len(self.qidxs)):
                ...
                while len(nidxs) < self.nnum:
                    ...
                    if not self.clusters[potential] in clusters:
                        ...
                        # 这里的计算全是 CPU Tensor，所以 avg_ndist 也必须是 CPU
